...
void personType::get_record(ifstream& inFile){
	inFile >> firstName >> lastName >> accountNumber;
	inFile >> tempType;
...
//Public:
bool personType::check_searchName(string name1) {
	if(lastName == name1) {
		return true;
	}
	else
...
float personType::sum_dollars() {
	float totalSum += amount;
	return totalSum;
}
