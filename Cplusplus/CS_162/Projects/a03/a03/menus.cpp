...
void access_menu(char selection, personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog,ofstream& logFile) {
	switch(selection) {
	case 1: 
...
void customer_interface(personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
	int option = 0;
	...
	//	Customer functions.
void customer_functions(int option, personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
		switch(option) {
		case 1:
...
void employee_interface(personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
	int option = 0;
	...
	//	Employee Functions.
void employee_functions(int option, personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
	switch(option) {
		case 1:
			withdraw_funds(database, withdrawlLog, logFile);
			break;
...
void manager_interface(personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
	int option = 0;
	string outName;
	...
	//	Manager Functions.
void manager_functions(int option, personType database[], int counter[], ofstream& depositLog, ofstream& withdrawlLog, ofstream& logFile) {
	switch(option) {
		case 1:
			withdraw_funds(database, withdrawlLog, logFile);
			break;
...
void add_person(personType database[], int counter[]) {
	int i = 0;
	int input = 0;
...
void database_dump(personType database[], int counter[]) {
	int count = 0;
	ofstream outFile;
	outFile.open("database.txt");

	for(int i = 0; i < counter[count]; i++) {
		database[i].print_record(outFile);
	}
	outFile.close();
}

	//Function to read in database from text file
int database_load(personType database[]) {
	int i = 0;
	...
	while(!inFile.eof()) {
		...
		database[i].set_record(tempFName, tempLName, tempAccount, tempType, tempAmount);
		i++;
	}	//	Ends while loop.
...
int find_customer_index(personType database[]) {
	int index = -1;
	string account;
	bool yes = false;
	...
	for(int i = 0; i < MAX_ACCOUNTS; i++) {
		yes = database[i].index_search(account);
		if(yes)
		{
			index = i;
			break;
...
void deposit_funds(personType database[], ofstream& depositLog, ofstream& logFile) {
	float amount1 = 0.0;
	...
	float printAmt = database[index].call_deposit(amount1);

	cout << "\nYou deposited: $" << fixed << showpoint << setprecision(2) << amount1 << endl;
	cout << "The new balance is: $" << printAmt << endl;

	database[index].deposit_log(depositLog, amount1);
	database[index].log_file(logFile, amount1, transType);
}

	//	Function to withdraw funds from account.
void withdraw_funds(personType database[], ofstream& withdrawlLog, ofstream& logFile) {
	float amount1 = 0.0;
	...
	float printAmt = database[index].call_withdrawl(amount1);

	cout << "\nYou withdrew: $" << fixed << showpoint << setprecision(2) << amount1 << endl;
	cout << "The new balance is: $" << printAmt << endl;

	database[index].withdrawl_log(withdrawlLog, amount1);
	database[index].log_file(logFile, amount1, transType);
}

	//	Function to print out account balance.
void balance_inquiry(personType database[]) {
	int index = find_customer_index(database);
	database[index].print_balance();
}

	//	Function to search by customer last name.
void search_customerName(personType database[]) {
	int index = -1;
	string name1;
	bool yes = false;
	...
	for(int i = 0; i < MAX_ACCOUNTS; i++) {
		yes = database[i].index_searchName(name1);
		if(yes)
		{
			index = i;
			break;
	...
	database[index].show_record();
}

	//	Function to transfer funds between accounts.
void xfer_funds(personType database[], ofstream& logFile, ofstream& withdrawlLog, ofstream& depositLog) {
	float amount1 = 0.0;
	...
	database[index1].call_withdrawl(amount1);
	database[index2].call_deposit(amount1);

	database[index1].print_balance();
	database[index2].print_balance();
	
	int transType = WITHDRAWL;
	database[index1].withdrawl_log(withdrawlLog, amount1);
	database[index1].log_file(logFile, amount1, transType);

	transType = DEPOSIT;
	database[index2].deposit_log(depositLog, amount1);
	database[index2].log_file(logFile, amount1, transType);
}

	//	Function to find transfer account(s).
int find_customer_index_for_transfer(personType database[], string msg) {
	int index = -1;
	string account;
	bool yes = false;
...
void list_dollars(personType database[], int counter[]) {
	float totalDollars = 0.0;
	float sum = 0.0;
	int count = 0;

	for(int i = 0; i < counter[count]; i++) {
		sum = database[i].get_sum();
		totalDollars += sum;
	}
...
void list_customers(personType database[], int counter[]) {
	int count = 0;

	cout << "\nCustomer Records by Name:" << endl;
	cout << endl;
	for(int i = 0; i < counter[count]; i++) {
		database[i].print_name();		
	}
}
...
void del_person(personType database[], int counter[]) {
	int index = 0;
	...
	database[index].del_record(deletedAccountNum);
}
