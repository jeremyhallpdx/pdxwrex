...
void database_dump(personType database[][MAX_ACCOUNTS], int counter[]) {
	...
	for(int i = 0; i < counter[count]; i++) {
		database[count][i].print_record(outFile);
	}
	outFile.close();
}
...
int database_load(personType database[][MAX_ACCOUNTS]) {
	...
	while(!inFile.eof()) {
		...
		database[recordType][i].set_record(tempFName, tempLName, tempAccount, tempType, tempAmount);
		i++;
	}	//	Ends while loop.
...
int find_customer_index(personType database[][MAX_ACCOUNTS]) {
	...
	for(int i = 0; i < MAX_ACCOUNTS; i++) {
		yes = database[CUSTOMER][i].index_search(account);
		if(yes)
		{
			index = i;
			break;
...
void deposit_funds(personType database[][MAX_ACCOUNTS], ofstream& depositLog, ofstream& logFile) {
	...
	float printAmt = database[CUSTOMER][index].call_deposit(amount1);

	cout << "\nYou deposited: $" << fixed << showpoint << setprecision(2) << amount1 << endl;
	cout << "The new balance is: $" << printAmt << endl;

	database[CUSTOMER][index].deposit_log(depositLog, amount1);
	database[CUSTOMER][index].log_file(logFile, amount1, transType);
}
...
void withdraw_funds(personType database[][MAX_ACCOUNTS], ofstream& withdrawlLog, ofstream& logFile) {
	...
	float printAmt = database[CUSTOMER][index].call_withdrawl(amount1);

	cout << "\nYou withdrew: $" << fixed << showpoint << setprecision(2) << amount1 << endl;
	cout << "The new balance is: $" << printAmt << endl;

	database[CUSTOMER][index].withdrawl_log(withdrawlLog, amount1);
	database[CUSTOMER][index].log_file(logFile, amount1, transType);
}
...
void balance_inquiry(personType database[][MAX_ACCOUNTS]) {
	int index = find_customer_index(database);
	database[CUSTOMER][index].print_balance();
}
...
void search_customerName(personType database[][MAX_ACCOUNTS]) {
	...
	for(int i = 0; i < MAX_ACCOUNTS; i++) {
		yes = database[CUSTOMER][i].index_searchName(name1);
		if(yes)
		{
			index = i;
			break;
	...
	database[CUSTOMER][index].show_record();
}

	//	Function to transfer funds between accounts.
void xfer_funds(personType database[][MAX_ACCOUNTS], ofstream& logFile, ofstream& withdrawlLog, ofstream& depositLog) {
	float amount1 = 0.0;
	...
	database[CUSTOMER][index1].call_withdrawl(amount1);
	database[CUSTOMER][index2].call_deposit(amount1);

	database[CUSTOMER][index1].print_balance();
	database[CUSTOMER][index2].print_balance();
	
	int transType = WITHDRAWL;
	database[CUSTOMER][index1].withdrawl_log(withdrawlLog, amount1);
	database[CUSTOMER][index1].log_file(logFile, amount1, transType);

	transType = DEPOSIT;
	database[CUSTOMER][index2].deposit_log(depositLog, amount1);
	database[CUSTOMER][index2].log_file(logFile, amount1, transType);
}
...
void del_person(personType database[][MAX_ACCOUNTS], int counter[]) {
	...
	database[input][index].del_record(deletedAccountNum);
}
