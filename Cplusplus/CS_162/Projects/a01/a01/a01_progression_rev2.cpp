...
accountType type;
//	Global Constants.
const int SIZE = 50;			//	Constant for name sizes.
const int MAX_PEOPLE_TYPE = 2;	
...
int find_customer(personType[][MAX_ACCOUNTS]);
...
int database_load(personType database[][MAX_ACCOUNTS]) {
	...
	while(!inFile.eof()) {
		++j;
		inFile >> database[capacity][i].firstName >> database[capacity][i].lastName
			>> database[capacity][i].accountNumber >> database[capacity][i].amount;
		inFile >> tempType;
...
int find_customer(personType database[][MAX_ACCOUNTS]) {
	int index = -1;
	string account;
