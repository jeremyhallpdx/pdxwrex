...
const int DEPOSIT = 0;				//	Constant for deposits in transactions[][MAX_TRANSACTIONS].
const int WITHDRAW = 1;				//	Constant for withdrawls in transactions[][MAX_TRANSACTIONS].
const int TRANSFER = 3;				//	Constant for transfers in transactions[][MAX_TRANSACTIONS].
...
struct transactionRecord {
	string date;					//	To be entered as: 01012009 for January 1, 2009.
	string accountNum;
	float amt;
};*/
...
/*	//	Funcs for Menu's:
void main_menu();
void access_menu(char, personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
void customer_interface(personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
void employee_interface(personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
void manager_interface(personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);

	//	Funcs to get user inputs:
int get_option();

	//	Funcs for operations:
void customer_functions(int, personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
void employee_functions(int, personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
void manager_functions(int, personType[][MAX_ACCOUNTS], int[], transactionRecord[][MAX_TRANSACTIONS]);
...
void deposit_funds(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);
void withdraw_funds(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);
void balance_inquiry(personType[][MAX_ACCOUNTS]);
void xfer_funds(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);
void add_person(personType[][MAX_ACCOUNTS], int[]);
//void del_customer(persontType[][], int[]);
...
//	void list_deposits(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);
//	void list_withdrawls(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);
//	void list_transactions(personType[][MAX_ACCOUNTS], transactionRecord[][MAX_TRANSACTIONS]);*/

	//	Main:
int main() {												
	personType database[PEOPLE_TYPES][MAX_ACCOUNTS];		//	Array of structs to contain database records and the type of record.
	transactionRecord transactions[TRANSACTION_TYPES][MAX_TRANSACTIONS];		//	Array to store transaction log data.
	...
	do {
		...
		access_menu(selection, database, counter, transactions);
	}while(selection != 4);
...
void access_menu(char selection, personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	switch(selection) {
	case 1: 
		customer_interface(database, counter, transactions);
		break;
	case 2: 
		employee_interface(database, counter, transactions);
		break;
	case 3: 
		manager_interface(database, counter, transactions);
		break;
	case 4:
...
void customer_interface(personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	int option = 0;
	...
	do {
		...
		option = get_option();
		customer_functions(option, database, counter, transactions);
	}while(option != 5);
}


	//	Customer functions.
void customer_functions(int option, personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
		switch(option) {
		case 1:
			withdraw_funds(database, transactions);
			break;
		case 2:
			deposit_funds(database, transactions);
			break;
		case 3:
			xfer_funds(database, transactions);
			break;
		case 4:
...
void employee_interface(personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	int option = 0;
	...
	do {
		...
		option = get_option();
		employee_functions(option, database, counter, transactions);
	}while(option != 8);
}

	//	Employee Functions.
void employee_functions(int option, personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	switch(option) {
		case 1:
			withdraw_funds(database, transactions);
			break;
		...
		case 5:
			...
			break;
		case 6:
			cout << "del_customer(database, counter);";
			break;
		case 7:
...
void manager_interface(personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	int option = 0;
	string outName;
	...
	do {
		...
		option = get_option();
		manager_functions(option, database, counter, transactions);
	}while(option != 13);
}

	//	Manager Functions.
void manager_functions(int option, personType database[][MAX_ACCOUNTS], int counter[], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	switch(option) {
		case 1:
			withdraw_funds(database, transactions);
			break;
		...
		case 9:
			...
			break;
		case 10:
			cout << "list_deposits(database, counter, transactions);" << endl;
			break;
		case 11:
			cout << "list_withdrawls(database, counter, transactions);" << endl;
			break;
		case 12:
			cout << "list_transactions(transactions);" << endl;
			break;
		case 13:
...
void deposit_funds(personType database[][MAX_ACCOUNTS], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	float amount1 = 0.0;
...
void withdraw_funds(personType database[][MAX_ACCOUNTS], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	ofstream logFileSub;
	float amount1 = 0.0;
	int index = find_customer_index(database);
	...
	logFileSub.open("withdrawl_log.txt");

	logFileSub << fixed << showpoint << setprecision(2)
...
void xfer_funds(personType database[][MAX_ACCOUNTS], transactionRecord transactions[][MAX_TRANSACTIONS]) {
	float amount1 = 0.0;
